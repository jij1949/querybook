...
# EXTRACT_REGEX = r"extract\(\s*(day|dayofweek|hour|minute|month|quarter|second|week|year)\s* from (([\w ()]+)|(\s*\'[" \
#                 r"\w\s\-\:]*\'\s*))\) "
EXTRACT_REGEX = r"extract\(\s*(dayofweek)\s* from (([\w ()]+)|(\s*\'[" \
                r"\w\s\-\:]*\'\s*))\) "
DATE_SUB_REGEX = r"date_sub\((([\w ()]+)|(\s*\'[\w\s\-\:]*\'\s*)),\s*(\-*\d+)\s*\)"
DATE_ADD_REGEX = r"date_add\((([\w ()]+)|(\s*\'[\w\s\-\:]*\'\s*)),\s*(\-*\d+)\s*\)"
...
CREATE_EXTERNAL_TABLE_REGEX = "\s*CREATE\s+EXTERNAL\s+TABLE\s*"
CREATE_TABLE = "CREATE TABLE "
STRING = "string"
VARCHAR = "VARCHAR"
#COALESCE_VARCHAR_REGEX=r"COALESCE\(([\'*\D*]+),\s*(\D*)\s*\)"
COALESCE_VARCHAR_REGEX=r"COALESCE\((([\w ()\.]+)|(\s*\'[\w\s\-\:]*\'\s*)),(([\w ()]+)|(\s*\'[\w\s\-\:]*\'\s*))\)"
COALESCE_INT_REGEX=r"COALESCE\(([\d*]+),\s*(\D*)\s*\)"
