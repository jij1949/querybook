...
export const SharedBoardPage: React.FunctionComponent = () => {
    ...
    const loadMoreBoards = React.useCallback(async () => {
        ...
        setLoading(true);
        try {
            const response = await BoardResource.getAllShared(
                environmentId,
                userId,
                BOARDS_PER_PAGE,
                offset.current
    ...
    React.useEffect(() => {
        if (environmentId && userId && boardIds.length === 0 && !loading) {
            offset.current = 0; 
            loadMoreBoards();
        }
    }, [environmentId, userId, loadMoreBoards]);

    // Reset state when environment or user changes
    React.useEffect(() => {
        setBoardIds([]);
        setHasMore(true);
        offset.current = 0;
    ...
    const boardItemRenderer = React.useCallback((boardId: number) => (
        <BoardBoardItem boardId={boardId} key={boardId} />
    ), []);

    const boardItemDOM = boardIds.length === 0 ? (
        <EmptyText className="m24">No shared lists found.</EmptyText>
    ) : (
        <div>
