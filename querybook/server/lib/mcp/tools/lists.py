...
from logic.board import (
    ...
    get_all_shared_boards,
    get_my_editable_boards,
...
def register(mcp: FastMCP) -> None:
    ...
    def list_lists(
        ...
        scope: Annotated[
            Literal["mine", "shared", "public", "editable"],
            "Which lists to return: 'mine' (owned by you), 'shared' (shared with you), 'public' (all public), 'editable' (lists you can edit).",
        ] = "mine",
        token: AccessToken = CurrentAccessToken(),
    ) -> list[dict]:
        ...
        with DBSession() as session:
            if scope == "mine":
                ...
                lists = get_all_shared_boards(environment_id, uid, session=session)
            elif scope == "public":
                lists = get_all_public_boards(environment_id, session=session)
            elif scope == "editable":
                lists = get_my_editable_boards(environment_id, uid, session=session)
            else:
