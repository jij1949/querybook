    ...
    def __init__(
        ...
        if thrift_transport is not None:
            ...
            if "EXTRA_USE_SSL" in configuration.keys():
                socket = thrift.transport.TSSLSocket.TSSLSocket(
                    host, port, cert_reqs=CERT_NONE
                )
                # Remove this key from the configuration dictionary
                # so it doesn't get passed to the server
